...
function parseRange(query) {
  const start = Math.max(1, Number.parseInt(first(query.start) || "1", 10) || 1);
  const requestedEnd = Number.parseInt(first(query.end) || String(start + 11), 10) || start;
...
function fetchCulturalEvents(query = {}) {
  ...
  const cacheKey = `${start}|${end}|${first(query.code) || ""}|${first(query.title) || ""}|${first(query.date) || ""}`;
  ...
  return new Promise((resolve, reject) => {
    ...
    .catch((error) => {
      if (query._retried || error.statusCode) throw error;
      return fetchCulturalEvents({ ...query, _retried: true });
    });
}
